...
const UI_CONFIG = {
  ANIMATION: {
    ...
    FADE_IN: 'animate-in fade-in slide-in-from-bottom-2 duration-300',
    FADE_IN_TEXT: 'animate-in fade-in slide-in-from-bottom-1 duration-300',
  },
  COLORS: {
    PRIMARY: 'blue',
    SUCCESS: 'green',
...
// КОМПОНЕНТ
// =====================================================
export const ServerInfoCard = ({ serverVersion, serverIp, onCopy, copied }: ServerInfoProps) => {
  // Упрощенная обработка нажатия (вся логика в родительском компоненте)
  const handleCopy = () => {
    onCopy();
  };
      ...
      <div className="relative flex items-center justify-center gap-4 mb-6">
        <div
          className={cn(
            ...
            'bg-gradient-to-br from-blue-700/30 to-blue-600/30',
            'p-2.5 shadow-lg shadow-blue-900/20',
            UI_CONFIG.ANIMATION.PULSE_DURATION
          )}
          aria-hidden="true"
        >
        ...
        <span className="font-bold text-white text-lg">
          ...
          <span className="text-blue-400 relative">
            {serverVersion}
            <span className="absolute bottom-0.5 left-0 w-full h-px bg-gradient-to-r from-transparent via-blue-400/50 to-transparent" />
          </span>
        </span>
      </div>

      {/* Копирование IP */}
      <div className="bg-gradient-to-t from-slate-700/90 to-slate-800/90 px-6 py-4 rounded-lg mb-2 border border-slate-700/50 shadow-inner">
        ...
        <button
          className={cn(
            ...
            'cursor-pointer p-3 rounded-lg group',
        ...
        >
          ...
          >
            <div
              className={cn(
                ...
                '[transform-origin:center] group-hover:scale-150'
              )}
            />
          </div>
          ...
          <span
            className={cn(
              'font-bold text-blue-400 mr-2 text-lg tracking-wide relative z-10',
          ...
          <div className="relative z-10 transition-transform group-hover:scale-110 duration-300">
            {copied ? (
              <span className="flex items-center text-green-400 gap-1">
                <Check size={20} className={UI_CONFIG.ANIMATION.FADE_IN} />
                <span className={cn('text-sm', UI_CONFIG.ANIMATION.FADE_IN_TEXT)}>
                  Скопировано
                </span>
              </span>
            ) : (
